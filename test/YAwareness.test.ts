...
describe("YAwareness", () => {
  ...
  describe("local lens", () => {
    it("setLocal and getLocal round-trip", () => {
      const doc = new Y.Doc()
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      expect(handle.local.syncGet()).toEqual(alice)
      handle.awareness.destroy()
    })
    ...
    it("focus into nested fields and read", () => {
      ...
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      expect(handle.local.focus("cursor").syncGet()).toEqual({ x: 10, y: 20 })
      expect(handle.local.focus("cursor").focus("x").syncGet()).toBe(10)
      expect(handle.local.focus("user").focus("name").syncGet()).toBe("Alice")
      handle.awareness.destroy()
    })
    ...
    it("focus into nested field and set", () => {
      ...
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      handle.local.focus("cursor").focus("x").syncSet(100)
      expect(handle.local.focus("cursor").focus("x").syncGet()).toBe(100)
      // Other fields preserved
      expect(handle.local.focus("cursor").focus("y").syncGet()).toBe(20)
      expect(handle.local.focus("user").focus("name").syncGet()).toBe("Alice")
      handle.awareness.destroy()
    })
    ...
    it("focus set on struct replaces the substruct", () => {
      ...
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      handle.local.focus("cursor").syncSet({ x: 99, y: 88 })
      expect(handle.local.focus("cursor").syncGet()).toEqual({ x: 99, y: 88 })
      handle.awareness.destroy()
    })
    ...
    it("throws TypedYValidationError on invalid set", () => {
      const doc = new Y.Doc()
      const handle = YAwareness.make(PresenceSchema, doc)
      expect(() => handle.local.syncSet({ user: 123 } as any)).toThrow(TypedYValidationError)
      handle.awareness.destroy()
    })
    ...
    it("throws TypedYValidationError on invalid focused set", () => {
      ...
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      expect(() => handle.local.focus("cursor").focus("x").syncSet("not a number" as any)).toThrow(
    ...
    it("get returns initial empty state before set", () => {
      ...
      expect(handle.local.syncGet()).toEqual({})
      handle.awareness.destroy()
    })
  ...
  describe("clearLocal", () => {
    it("sets state to null via Effect", () => {
      ...
      expect(handle.local.syncGet()).toBeNull()
      handle.awareness.destroy()
    })

    it("syncClearLocal sets state to null", () => {
      const doc = new Y.Doc()
      const handle = YAwareness.make(PresenceSchema, doc)
      handle.local.syncSet(alice)
      expect(handle.local.syncGet()).toEqual(alice)
      handle.syncClearLocal()
      expect(handle.local.syncGet()).toBeNull()
      handle.awareness.destroy()
    })
  })
  ...
  describe("remote lens", () => {
    it("reads remote client state", () => {
      ...
      expect(remote.syncGet()).toEqual(alice)
      a1.destroy()
      a2.destroy()
    })
    ...
    it("focus into remote state fields", () => {
      ...
      expect(h2.remote(a1.clientID).focus("cursor").syncGet()).toEqual({ x: 10, y: 20 })
      expect(h2.remote(a1.clientID).focus("cursor").focus("x").syncGet()).toBe(10)
      expect(h2.remote(a1.clientID).focus("user").focus("name").syncGet()).toBe("Alice")
      a1.destroy()
      a2.destroy()
    })
    ...
    it("returns undefined for unknown client", () => {
      const doc = new Y.Doc()
      const handle = YAwareness.make(PresenceSchema, doc)
      expect(handle.remote(999999).syncGet()).toBeUndefined()
      handle.awareness.destroy()
    })
  })
